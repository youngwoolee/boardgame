...
public class InitConfig {
    ...
    CommandLineRunner init(GameRepository gameRepository) {
        return args -> {
            ...
            games.addAll(createGames("스플렌더", "보석 상인이 되어 보석을 수집하고, 카드와 귀족을 구매해 점수를 얻는 전략형 보드게임입니다.",
                                     ...
                                     Arrays.asList("전략"), Arrays.asList("셋컬렉션", "카드드래프트"), "SPLEND", 1));
            ...
            games.addAll(createGames("아발론", "아발론은 선과 악의 세력이 대결하는 정체 은닉 기반의 심리전 게임입니다.",
                                     ...
                                     Arrays.asList("추리", "심리전", "파티"), Arrays.asList("정체은닉", "토론"), "AVALON", 5));
    ...
    private List<Game> createGames(String name, String description, String imageUrl, String tag,
                                   ...
                                   String baseBarcode, int quantity) {
        ...
        for (String genreName : genreNames) {
            try {
                ...
                System.out.println("존재하지 않는 장르: " + genreName);
            }
        }

        Set<SystemType> systems = new HashSet<>();
        for (String systemName : systemNames) {
            try {
                systems.add(SystemType.valueOf(systemName));
            } catch (IllegalArgumentException e) {
                System.out.println("존재하지 않는 시스템: " + systemName);
            }
        }
